 ...
 * whole structure.
 *
 * <p> A node in a {@code LinkedList} represent a single element of type
 * {@link E}, alongside a reference that points to the next node in the list
 * and, although optional, a reference that points to the previous node in the
 * list. Altogether, they become a chain of nodes linked together in a linear
 * fashion.
 *
 * <p> Nodes in a {@code LinkedList} accepts all elements, including duplicates
 * and {@code null}, as long as they follow the constraints specified at {@link
 * E}.
 *
 * <p> Due to its structure, retrieving elements become less efficient as the
 * user must traverse through all the nodes in the list until the node to be
 ...
 * of the whole list.
 *
 * <p> All <code>LinkedList</code> objects should implement the {@link Iterable}
